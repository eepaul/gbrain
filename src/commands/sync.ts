...
async function performSyncInner(engine: BrainEngine, opts: SyncOpts): Promise<SyncResult> {
  ...
  // (guard applied inside the loop below).
  //
  // v0.41.15.0: filter widened from `!isSyncable` to `!isAllowed` so a file
  // that became newly excluded by a .gbrainignore / sources.config /
  // --exclude pattern (and shows up in the modified set because it was
  // edited concurrently) also gets its stale page reaped. Pages that don't
  // appear in the diff manifest at all are handled by the reconciliation
  // pass below. The metafile guard inside the loop still applies because
  // we iterate the filtered set and short-circuit on `unsyncableReason`.
  //
